...
@frappe.whitelist()
def get_report_data(filters=None, selected_filters=None):
    ...
    data = get_data(filters, selected_filters)
    chart_data = get_chart_data(data)

    return {"columns": columns, "data": data, "chart_data": chart_data}
...
def get_columns():
    return [
        ...
        {"fieldname": "real_emission_cost", "fieldtype": "Data", "label": "Actual Cost", "width": 150},
        {"fieldname": "standard_emission_cost", "fieldtype": "Data", "label": "Standard Cost", "width": 150},
    ]


def get_data(filters=None, selected_filters=None):
    filters = filters or {}
    selected_filters = selected_filters or {}

    # Merge selected_filters into filters, but only non-empty values
    for key, value in selected_filters.items():
        # Skip empty strings, None, empty arrays, and zero values for non-numeric fields
        if value is not None and value != '' and value != []:
            # For arrays, only include if they have actual values
    ...
    frappe.log_error("selected_filters", selected_filters)
    frappe.log_error("merged_filters", filters)
    ...
    # Get calculation values from selected_filters, default to 0.0 if not provided
    ...
    frappe.log_error("Calculation values", {
        "cbam_factor": cbam_factor,
        "bench_mark": bench_mark,
        "emission_value": emission_value,
        "ets_carbon_price": ets_carbon_price
    })
